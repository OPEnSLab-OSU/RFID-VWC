/*
  ...
  Date: March 4th, 2020
  Appended By: Natalie Clouse
  Date Febuary 25, 2021
...
#define WET 16 //neopixle
#define DRY 21 //neopixle
#define MAX_VALUE 25//dry was 27
#define MIN_VALUE 21//wet was 17
  ...
  if (nano.check() == true) //Check to see if any new data has come in from module
  {
    ...
    else if (responseType == RESPONSE_IS_TAGFOUND)
    {
      ...
      if(freq < 1000000 && nano.getAntennaeIDNew() == 17 && tagEPCBytes <= 20){
        ...
        Loom.add_data("Relay state","Relay state",Relay_state);
        Loom.SDCARD().log();
        Loom.pause();
...
void addNewTag(int i){
   ...
   newTag.threshold     = MIN_VALUE; //all tags initally at min value threshold? 
...
if (tag_array[index].rssi > RSSI_THRESHOLD)
 {
  if(tag_array[index].moisture >= MAX_VALUE) //dry
  {
   turnOnRelay();
   ...
   Serial.print(" WET_CYCLE state realy on");
  ...
  else if (tag_array[index].moisture < MAX_VALUE)//wet
  {
   turnOffRelay();
   ...
   Serial.print(" Current RSSI: ");
   Serial.print(tag_array[index].rssi);
   Serial.print(" DRY_CYCLE state relay off ");
   Serial.print("Current moisture: ");
   Serial.print(tag_array[index].moisture);
   Serial.print(" Threshold moisture: ");
   Serial.println(tag_array[index].threshold);
  ...
  else if (tag_array[index].moisture < MIN_VALUE)//wet
  {
   turnOffRelay();
   strcpy(Relay_state,"OFF");
   Serial.print("EPC Tag: ");
 ...
 else if (tag_array[index].rssi < RSSI_THRESHOLD)//to far away
 {
  turnOffRelay();
  strcpy(Relay_state,"OFF");
   Serial.print("EPC Tag: ");
